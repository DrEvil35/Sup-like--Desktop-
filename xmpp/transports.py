...
from binding import bindingUp,IN,OUT
import select
import socket
import sys
...
class TCPSocket(plugin.PlugIn):
	...
	def __init__(self, useResolver=True):
		self.streamStatIN = 0
		self.streamStatOUT = 0
		plugin.PlugIn.__init__(self)
		self.debugFlag = DBG_SOCKET
		self._exportedMethods = (self.send, self.disconnect,self.streamStat)
	...
	def receive(self):
		...
		recvData = len(received)
		if recvData:
			self.streamStatIN += recvData
			self._seen_data = 1
		else:
	...
	def send(self, rawdata):
		...
		"""
		try:
			self.streamStatOUT+=len(rawdata)
			self._send(rawdata)
	...
	def disconnect(self):
		...
		self._sock.close()
	def streamStat(self):
		return {'incoming':self.streamStatIN,
			'outgoing':self.streamStatOUT,
			'all':self.streamStatIN+self.streamStatOUT}
