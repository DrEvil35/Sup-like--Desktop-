		...
		if self.namespace:
			if not self.parent or self.parent.namespace != self.namespace:
				if "xmlns" not in self.attrs:
					s.append(" xmlns=\"%s\"" % (self.namespace))
		s+=[" %s=\"%s\""%(key,escapeXML(ustr(self.attrs[key]))) for key in self.attrs.keys()]
		if not self.children and not self.data:
			s.append(" />")
		else:
			s.append(">")
			if self.children:
				s+=[ustr(child) for child in self.children if child]
			if self.data:
				s.append(escapeXML(self.getData().strip()))
